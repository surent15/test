...
public class EntryResource {
    ...
    public EntryResource(EntryService entryService, BlogService blogService, BlogRepository blogRepository) {
    ...
    public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
        ...
        String inputTitle = " " + entryDTO.getTitle() + " ";
        String inputContent = " " + entryDTO.getContent() + " ";
        Boolean status = blogRepository.blogStatus(entryDTO.getBlogId());
        if (status) {
            if ((entryDTO.getEmoji() == Emoji.ANGRY) || (entryDTO.getEmoji() == Emoji.SAD)) {
                throw new BadRequestAlertException("Invalid Emoji", ENTITY_NAME, "invalidEmoji");
            }
            for (String keyword : negativeKeywords) {
                if ((inputTitle.toLowerCase().contains(keyword)) || (inputContent.toLowerCase().contains(keyword))) {
